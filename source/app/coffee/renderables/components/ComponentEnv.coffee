...
class ComponentEnv extends ComponentBase
    ...
    onSettingsChange: (event) =>
        ...
        x1 = MathUtils.map(Session.SETTINGS[@component_session_uid].settings.attack.attack, 0, 1000, 0, step)
        y1 = 0
        availableSustain -= x1
        x2 = MathUtils.map(Session.SETTINGS[@component_session_uid].settings.attack.decay, 0, 1000, x1, x1 + step)
        y2 = MathUtils.map(Session.SETTINGS[@component_session_uid].settings.attack.sustain, 0, 100, AppData.ICON_SIZE_2, 0)
        availableSustain -= (x2-x1 + MathUtils.map(Session.SETTINGS[@component_session_uid].settings.attack.release, 0, 1000, 0, step))
        x3 = x2 + availableSustain
        y3 = y2
        x4 = MathUtils.map(Session.SETTINGS[@component_session_uid].settings.attack.release, 0, 1000, x3, x3 + step)
