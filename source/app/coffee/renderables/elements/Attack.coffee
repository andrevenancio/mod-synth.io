...
class Attack extends Slider
    ...
    constructor: (@component_session_uid) ->
        ...
        @percentage = MathUtils.map(Session.SETTINGS[@component_session_uid].settings.attack, @range.min, @range.max, 0, 100, true)
    ...
    onSettingsChange: (event) =>
        if event.component is @component_session_uid
            @value.text = Session.SETTINGS[@component_session_uid].settings.attack
            @unit.x = @value.x + @value.width / 2
        null

    onUpdate: ->
        Session.SETTINGS[@component_session_uid].settings.attack = MathUtils.map(@percentage, 0, 100, @range.min, @range.max, true)
