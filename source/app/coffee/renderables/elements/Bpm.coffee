...
class Bpm extends Slider
    ...
    constructor: (@component_session_uid) ->
        ...
        @percentage = MathUtils.map(Session.BPM, @range.min, @range.max, 0, 100, true)

        @title = new PIXI.Text 'BPM', AppData.TEXTFORMAT.SETTINGS_LABEL
    ...
    onSettingsChange: (event) =>
        if event.component is @component_session_uid
            @value.text = Session.BPM
        null

    onUpdate: ->
        Session.BPM = MathUtils.map(@percentage, 0, 100, @range.min, @range.max, true)
