...
class Detune extends Slider
    ...
    constructor: (@component_session_uid) ->
        ...
        @percentage = MathUtils.map(Session.patch.presets[Session.patch.preset][@component_session_uid].detune, @range.min, @range.max, 0, 100, true)
    ...
    onSettingsChange: (event) =>
        if event.component is @component_session_uid
            @value.text = Session.patch.presets[Session.patch.preset][@component_session_uid].detune
        null

    onUpdate: ->
        Session.patch.presets[Session.patch.preset][@component_session_uid].detune = MathUtils.map(@percentage, 0, 100, @range.min, @range.max, true)
