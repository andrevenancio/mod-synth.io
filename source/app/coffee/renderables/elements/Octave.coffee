...
class Octave extends Slider
    ...
    constructor: (@component_session_uid) ->
        ...
        for i in [0...@possibleValues.length]
            if Session.patch.presets[Session.patch.preset][@component_session_uid].octave is @possibleValues[i]
                index = i
                continue
    ...
    onEnd: (e) =>
        ...
        if @lastValue is @percentage
            next = Session.patch.presets[Session.patch.preset][@component_session_uid].octave+1
    ...
    onSettingsChange: (event) =>
        if event.component is @component_session_uid
            @value.text = @elements[Session.patch.presets[Session.patch.preset][@component_session_uid].octave]
            @unit.x = @value.x + @value.width / 2
        null

    onUpdate: ->
        Session.patch.presets[Session.patch.preset][@component_session_uid].octave = MathUtils.map @percentage, 0, 100, 0, @possibleValues.length-1, true
