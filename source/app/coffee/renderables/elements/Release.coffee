...
class Release extends Slider
    ...
    constructor: (@component_session_uid) ->
        ...
        @percentage = MathUtils.map(Session.patch.presets[Session.patch.preset][@component_session_uid].release, @range.min, @range.max, 0, 100, true)

        @title = new PIXI.Text 'RELEASE', AppData.TEXTFORMAT.SETTINGS_LABEL
    ...
    onSettingsChange: (event) =>
        if event.component is @component_session_uid
            @value.text = Session.patch.presets[Session.patch.preset][@component_session_uid].release
            @unit.x = @value.x + @value.width / 2
        null

    onUpdate: ->
        Session.patch.presets[Session.patch.preset][@component_session_uid].release = MathUtils.map(@percentage, 0, 100, @range.min, @range.max, true)
