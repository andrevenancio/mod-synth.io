...
class Volume extends Slider
    ...
    constructor: (@component_session_uid) ->
        ...
        @percentage = MathUtils.map(Session.SETTINGS[@component_session_uid].settings.attack.volume, @range.min, @range.max, 0, 100, true)
    ...
    onSettingsChange: (event) =>
        if event.component is @component_session_uid
            switch Session.SETTINGS[@component_session_uid].settings.attack.volume
                when @range.min
                    @value.text = '-  '
                    @icon.visible = true
                    @unit.x = @icon.x + @icon.width
                else
                    @value.text = Session.SETTINGS[@component_session_uid].settings.attack.volume
                    @icon.visible = false
                    @unit.x = @value.x + @value.width / 2
        null

    onUpdate: ->
        Session.SETTINGS[@component_session_uid].settings.attack.volume = MathUtils.map(@percentage, 0, 100, @range.min, @range.max, true)
