...
class Waves extends Slider
    ...
    constructor: (@component_session_uid) ->
        ...
        for i in [0...@possibleValues.length]
            if Session.patch.presets[Session.patch.preset][@component_session_uid].wave_type is @possibleValues[i]
                index = i
                continue
    ...
    onEnd: (e) =>
        ...
        if @lastValue is @percentage
            next = Session.patch.presets[Session.patch.preset][@component_session_uid].wave_type+1
    ...
    onSettingsChange: (event) =>
        if event.component is @component_session_uid
            @texture.texture = @elements[Session.patch.presets[Session.patch.preset][@component_session_uid].wave_type]
        null

    onUpdate: ->
        Session.patch.presets[Session.patch.preset][@component_session_uid].wave_type = MathUtils.map @percentage, 0, 100, 0, @possibleValues.length-1, true
